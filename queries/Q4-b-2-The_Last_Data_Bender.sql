-- 4b.2)
-- Re-implement your answer for questions 3-l using materialized views.
...
FROM (SELECT A1.mid as mid1, A2.mid as mid2, COUNT(A1.name) as common_actors_count
     ...
     (SELECT A3.mid, COUNT(A3.name) as total_actors_count
      FROM mw_actors as A3
      GROUP BY (A3.mid)) as total_count
WHERE common_count.mid1 = total_count.mid;

--  View creation time: 6ms

-- Create Materialized View with movie fraction of common tags
...
FROM (SELECT A1.mid as mid1, A2.mid as mid2, COUNT(A1.tid) as common_tid_count
      ...
      WHERE A1.mid <> A2.mid
        and A1.tid = A2.tid
      GROUP BY A1.mid, A2.mid) as common_count,
     (SELECT A3.mid, COUNT(A3.tid) as total_tid_count FROM mw_tags as A3 GROUP BY (A3.mid)) as total_count
WHERE common_count.mid1 = total_count.mid;
...
-- First execution time: 1min 44s 400ms
-- Subsequent execution time: 1min 40s 213ms